...
class ShortClipsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # addon für FF
        freq_data = torch.fft.rfft(waveform, n=self.max_length)
        freq_data = freq_data.abs()  # nur Magnitude
        
        # Label + Waveform zurückgeben
        return freq_data, label
...
class AudioCNN1D(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("Shape vor fc1:", x.shape) debugging relic
        x = self.fc1(x)
        x = torch.relu(x)
        x = self.fc2(x)
        return x
...
def main():
    ...
    # Modell speichern
    torch.save(model.state_dict(), "ML_CNN_wav_FF_faltung_netz.pth")
